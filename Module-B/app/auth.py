...
def login(username: str, password: str) -> dict:
    """
    Thread-safe login handler using RLock for concurrent request handling
    """
    # Acquire lock for thread-safe database operations during login
    auth_lock = current_app.config.get("THREAD_LOCKS", {}).get("auth", threading.RLock())
...
def verify_session(session_id: str, mac: str) -> dict:
    ...
    # Acquire READ lock for thread-safe session lookup
    session_lock = current_app.config.get("THREAD_LOCKS", {}).get("session", threading.RLock())
